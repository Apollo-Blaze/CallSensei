...
ipcMain.handle('github-login', async (_event, code) => {
...
function applyUnifiedDiff(baseDir, patchText) {
    const filesWritten = [];
...
ipcMain.handle('patch:preview', async (_event, args) => {
    const { patchText } = args;
    // Basic preview: list of paths affected
    const fileMatches = [...patchText.matchAll(/^\+\+\+\s+b\/(.*)$/gm)].map(m => ({ path: m[1], status: 'modify' }));
    const newFiles = [...patchText.matchAll(/^new file mode .*\nindex.*\n--- \/dev\/null\n\+\+\+ b\/(.*)$/gm)].map(m => ({ path: m[1], status: 'add' }));
    const deleted = [...patchText.matchAll(/^deleted file mode .*\nindex.*\n--- a\/(.*)\n\+\+\+ \/dev\/null$/gm)].map(m => ({ path: m[1], status: 'delete' }));
    const files = [...newFiles, ...deleted, ...fileMatches];
    return { ok: true, files };
});
ipcMain.handle('patch:apply', async (_event, args) => {
    const { patchText } = args;
...
ipcMain.handle('fs:selectFile', async (_event, args) => {
...
ipcMain.handle('terminal:spawn', async (event, args) => {
...
ipcMain.handle('terminal:kill', async (_event, args) => {
    ...
    if (!child)
        return { ok: false, message: 'No process found for session' };
    try {
...
ipcMain.handle('terminal:write', async (_event, args) => {
    const child = terminalProcesses.get(args.sessionId);
    if (!child?.stdin)
        return { ok: false, message: 'No stdin available' };
    try {
