...
export default function TerminalAndPatchView({ activityId }: TerminalAndPatchViewProps) {
    ...
    const writeAiContext = useCallback((next: Partial<{ terminalOutput: string; editorFilePath: string; editorFileContent: string }>) => {
        if (!activityId) return
        const key = `callsensei.aiContext.${activityId}`
        try {
            const existingRaw = localStorage.getItem(key)
            const existing = existingRaw ? JSON.parse(existingRaw) : {}
            const merged = { ...existing, ...next, updatedAt: Date.now() }
    ...
    return (
        <div className="flex flex-col gap-4 p-4 bg-[#0f0f1e] min-h-screen">
            ...
            <TerminalPanel
                ...
                onOutputSnapshot={(outputText: string) => writeAiContext({ terminalOutput: outputText })}
                initialCwd=""
            />
            ...
            */}
            <PatchReviewWithBoundary preselectedFile={pendingFilePath} activityId={activityId ?? undefined} />
